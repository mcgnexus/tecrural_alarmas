...
export default async function FitosanitarioPage() {
  ...
  const accesoAdmin = await verificarAccesoAdmin(new Request("https://tecrural.local/fitosanitario", { headers: cabeceras }));
  ...
  return <>
    ...
    {accesoAdmin.ok ? <section><h2 className="mb-2 text-xl font-extrabold text-stone-900">Aviso oficial</h2><ListaAvisosFitosanitarios /></section> : <section className="rounded-2xl border-2 border-stone-200 bg-stone-50 p-5"><h2 className="text-xl font-extrabold text-stone-900">Avisos de tu explotación</h2><p className="mt-1 text-base text-stone-700">Los avisos fitosanitarios se muestran asociados a la zona y los cultivos seleccionados en tus parcelas.</p></section>}
