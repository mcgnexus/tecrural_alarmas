      ...
      {errores.telefono ? <p className="mt-1 text-sm font-semibold text-red-700">{errores.telefono}</p> : null}
       ...
       <label className="mt-4 flex min-h-[52px] cursor-pointer items-start gap-3 rounded-xl border-2 border-stone-300 p-3">
          <input type="checkbox" required checked={acepta} onChange={(e) => { const valor = e.target.checked; setAcepta(valor); actualizarError("privacidad", valor); }} className="h-6 w-6 shrink-0 accent-brand-800" />
           <span className="text-[15px] font-medium text-stone-900"><strong>Necesario:</strong> acepto el tratamiento de mis datos para gestionar la solicitud y recibir avisos por WhatsApp. He leído la <Link href="/privacidad" className="inline-flex min-h-11 items-center py-2 font-bold text-brand-800 underline">política de privacidad</Link>.</span>
       </label>
        ...
        <details className="mt-3 rounded-xl border border-stone-200 bg-stone-50 p-3 text-sm leading-relaxed text-stone-700">
          ...
          <div className="mt-2 space-y-1">
            ...
            <p><strong>Para qué:</strong> crear la parcela y activar los avisos que solicitas.</p>
             <p><strong>Cancelación:</strong> escribe a {EMAIL_CONTACTO} para darte de baja o borrar tus datos. Consulta la <Link href="/privacidad" className="inline-flex min-h-11 items-center py-2 font-bold text-brand-800 underline">política de privacidad</Link>.</p>
